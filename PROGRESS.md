...
## 次にやること (Top 3)
1. **実際の会議で通す** — 残る最大の未検証。Teams の「通信デバイス」問題もここで出る。
   **録音音量の確認を録る前に**（デバイスごとに別設定・余裕は 2.2 dB）
2. **バッファ1秒で underrun が減ったか確認** — 11秒の録音では 0 回だったが短すぎて判らない。
   1の会議録音でそのまま確かめられる
3. **VAD は本番では使わない**（結論済み）。非常口で使うなら `-vsd 1000 -vp 200` を起点に
   CPU で実測する。幻聴の抑制効果は実会議の録音が要る
